...
socket.on("data", message => {
	...
	for (let i in test) {
		if (i == "roomdata") updateRoomData(test)
		else if (i == "roomexits") updateRoomExits(test)
		else if (i == "mapdata") updateMap(test);
		else if (i == "whodata") updateWho(test);
	}
})

socket.on("whochanged", () => {
	var messageBody = JSON.stringify({ "command": "getwhodata" });
...
function sendToGlobal(messageBody) {
	...
	if (newBody.startsWith("[You]:")) { bodyClass = "message fromyou"; }
...
function moveRoom(dir) {
	var messageBody = JSON.stringify({ "command": dir });
	socket.emit("data", messageBody);
}
...
document.getElementById("sendbutton").onmousedown = function() {
	...
	const messageBody = JSON.stringify({ "command": sendInput });
	socket.emit("data", messageBody);
